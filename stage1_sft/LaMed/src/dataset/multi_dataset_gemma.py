    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                question = 'user\n'+ image_prompt + '<start_of_turn>' + self.image_tokens + '<end_of_turn>'  + medium_prompt + prompt_question + '\nassistant\n'
...
class WeightCapDataset(Dataset):
    def __init__(self, args, tokenizer, mode="train"):
        ...
        # 预存特殊标记的 ID
        self.im_start_id = tokenizer.convert_tokens_to_ids("<start_of_turn>")
        self.im_end_id = tokenizer.convert_tokens_to_ids("<end_of_turn>")
    ...
    def build_chatml_prompt_with_weights(self, tokenizer, image_tokens, prompt_question, finding_list, impression_list):
        ...
        im_end_token = tokenizer.encode("<end_of_turn>", add_special_tokens=False)
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                # 获取 input_ids 中的 <start_of_turn> 位置
...
class WeightFindingCapDataset(Dataset):
    ...
    def build_chatml_prompt_with_weights(self, tokenizer, image_tokens, prompt_question, finding_list, impression_list):
        ...
        user_prompt = f"{prompt_question.strip()}{image_tokens}"
        ...
        # 构建 token 索引和权重
        finding_tokens, finding_weights = self.format_tag_block_with_weight(tokenizer, "finding", finding_list)
        im_end_token = tokenizer.encode("<end_of_turn>\n", add_special_tokens=False)
...
class ReportVQADataset(Dataset):
    ...
    def build_vqa_prompt_with_weights(self, tokenizer, image_tokens, prompt_question, finding_list, impression_list):
        ...
        user_prompt = f"{prompt_question}{image_tokens}"
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                user_prompt = f"{question}{self.image_tokens}"
                assistant_response = f"{answer}"
                # --- ChatML 对话构造 ---
                conversation = [
                    {"role": "system", "content": system_prompt},
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                question = 'user\n'+  image_prompt + '<start_of_turn>' + self.image_tokens + '<end_of_turn>' + ' ' + medium_prompt + ' '+ QUESTION_TEMPLATE.format(Question=question.format(sentences=object_location))  + '\nassistant\n'
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                """构建符合多模态对话格式的prompt"""
                question = 'user\n'+  image_prompt + '<start_of_turn>' + self.image_tokens + '<end_of_turn>' + medium_prompt + example['question'] + ' ' + CoTprompt + '\nassistant\n'
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                question = 'user\n'+  image_prompt + '<start_of_turn>' + self.image_tokens + '<end_of_turn>' + ' ' + medium_prompt + ' '+ QUESTION_TEMPLATE.format(Question=question.format(sentences=self.location_map[location_keys]))  + '\nassistant\n'
