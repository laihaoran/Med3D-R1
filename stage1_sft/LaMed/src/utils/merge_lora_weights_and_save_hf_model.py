...
from LaMed.src.model.language_model import LamedLlamaForCausalLM, LamedQwen2ForCausalLM
...
class ModelArguments:
    ...
    set_proj_num: int = field(default=256, metadata={"help": "Number of projectors in Perceiver."})
    use_random: bool = field(default=False, metadata={"help": "Use random projector."})
    random_num: int = field(default=1, metadata={"help": "Number of random projector."})
...
def main():
    ...
    # , "<|finding|>" , "<|impression|>","<|/finding|>", "<|/impression|>"
    # Define and add special tokens
    # special_token = {"additional_special_tokens": ["<im_patch>", "<bx_start>", "<bx_end>", "<|im_start|>", "<|im_end|>"]}
    ...
    elif 'qwen2' in model_args.model_type:
        model = LamedQwen2ForCausalLM.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir
        )
    elif 'qwen3' in model_args.model_type:
        model = LamedQwen3ForCausalLM.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir
        )
    ...
    if training_args.lora_enable:
        ...
        # modules_to_save=["lm_head"],  # <- 显式训练并保存
