...
from LaMed.src.model.language_model import LamedLlamaWoVisionForCausalLM
...
def extract_and_save_weights(state_dict, prefix_to_remove, output_path):
    """
    Extracts parameters from the state_dict that start with a specific prefix,
    removes the prefix, and saves the modified state_dict to a new file.

    Args:
        state_dict (dict): The original state_dict with parameters.
        prefix_to_remove (str): The prefix to remove from parameter names.
        output_path (str): Path to save the new state_dict.
    """
    # Create a new dictionary to hold the modified state_dict
    new_state_dict = {}

    # Iterate over the original state_dict
    for key, value in state_dict.items():
        # Check if the key starts with the desired prefix
        if key.startswith(prefix_to_remove):
            # Remove the prefix from the key
            new_key = key[len(prefix_to_remove):]
            # Add the modified key-value pair to the new state_dict
            new_state_dict[new_key] = value

    # Save the new state_dict to the specified output path
    torch.save(new_state_dict, output_path)
    print(f"New state_dict saved to {output_path}")
...
def main():
    ...
    prefix_to_remove = 'model.vision_tower.vision_tower.'
    output_path = 'vision_model_weights-31.pth'  # Specify the path where you want to save the new state_dict

    # Call the function to extract and save the weights
    extract_and_save_weights(state_dict, prefix_to_remove, output_path)
    ...
    if 'llama' in model_args.model_type:
        model = LamedLlamaWoVisionForCausalLM.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir
        )
    ...
    if model_args.vision_tower is not None:
        model.get_model().initialize_mm_projector(model_args=model_args)
    ...
    # torch.save(state_dict, os.path.join(training_args.output_dir, 'merged_model.bin'))  # don't save merged_model
